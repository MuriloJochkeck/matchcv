...
export async function POST(request: Request) {
  ...
  const { data: expired, error } = await admin.from("resumes").select("id, storage_key").not("expires_at", "is", null).lte("expires_at", new Date().toISOString()).is("deleted_at", null).limit(100);
  ...
  const keys = (expired ?? []).map((resume) => resume.storage_key).filter((key): key is string => typeof key === "string" && key.length > 0);
  if (!ids.length) return Response.json({ expired: 0, marked: 0, filesRemoved: 0 });
  if (keys.length) {
    const { error: storageError } = await admin.storage.from("resumes").remove(keys);
    if (storageError) return Response.json({ code: "RETENTION_STORAGE_FAILED", message: "Não foi possível remover arquivos expirados." }, { status: 500 });
  }
  ...
  return Response.json({ expired: ids.length, marked: marked?.length ?? 0, filesRemoved: keys.length });
}
