...
export default async function NewAnalysisPage() {
  ...
  return <AnalysisWizard resumes={(data ?? []).map((item) => { const version = item.resume_versions?.sort((a, b) => b.version - a.version)[0]; return { id: item.id, originalName: item.original_name, extractedText: version?.extracted_text ?? "" }; })} />;
}