...
<section className="rounded-xl border border-[#dfe3dc] bg-white p-4"><h2 className="font-bold">Revise os requisitos detectados</h2><p className="mt-1 text-sm text-[#5b655e]">Confirme os itens que serão comparados. Ajuste o nome e o tipo quando necessário.</p>{requirements.length ? <div className="mt-4 space-y-3">{requirements.map((requirement) => <div className="grid gap-2 sm:grid-cols-[1fr_150px]" key={requirement.id}><input aria-label="Nome do requisito" className="field-input" value={requirement.title} onChange={(event) => setRequirements((current) => current.map((item) => item.id === requirement.id ? { ...item, title: event.target.value } : item))} /><select aria-label="Tipo do requisito" className="field-input" value={requirement.kind} onChange={(event) => setRequirements((current) => current.map((item) => item.id === requirement.id ? { ...item, kind: event.target.value as JobRequirement["kind"] } : item))}><option value="required">Obrigatório</option><option value="desirable">Desejável</option></select></div>)}</div> : <p className="mt-3 text-sm text-[#7f3028]">Nenhum requisito conhecido foi detectado.</p>}</section>
