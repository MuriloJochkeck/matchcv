...
import type { JobRequirement } from "@/contracts/job";
import { extractJobRequirements } from "@/contracts/job";
...
export function AnalysisWizard({ resumes }: { resumes: Resume[] }) {
  ...
  async function submit() {
    ...
    try {
      ...
      const result = await createAnalysis({ resumeId, job: { title, company, description, requirements }, acceptedTerms: true });
  ...
  return <div className="mx-auto max-w-3xl">
    ...
    <div className="card mt-7 space-y-5 p-6">
      ...
      <label className="field-label">Descrição da vaga<textarea className="field-input mt-2 min-h-48" value={description} onChange={(e) => { const value = e.target.value; setDescription(value); setRequirements(extractJobRequirements(value)); }} /></label>
<section className="rounded-xl border border-[#dfe3dc] bg-white p-4"><h2 className="font-bold">Revise os requisitos detectados</h2><p className="mt-1 text-sm text-[#5b655e]">Confirme os itens que serão comparados. Ajuste o nome e o tipo quando necessário.</p>{requirements.length ? <div className="mt-4 space-y-3">{requirements.map((requirement) => <div className="grid gap-2 sm:grid-cols-[1fr_150px]" key={requirement.id}><input aria-label="Nome do requisito" className="field-input" value={requirement.title} onChange={(event) => setRequirements((current) => current.map((item) => item.id === requirement.id ? { ...item, title: event.target.value, keywords: [event.target.value], synonyms: [] } : item))} /><select aria-label="Tipo do requisito" className="field-input" value={requirement.kind} onChange={(event) => setRequirements((current) => current.map((item) => item.id === requirement.id ? { ...item, kind: event.target.value as JobRequirement["kind"] } : item))}><option value="required">Obrigatório</option><option value="desirable">Desejável</option></select></div>)}</div> : <p className="mt-3 text-sm text-[#7f3028]">Nenhum requisito conhecido foi detectado.</p>}</section>
